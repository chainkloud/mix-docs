...
Validators are also responsible for Mix's most crucial type of governance: adopting the network protocol changes via [Mix Improvement Proposals](fuse-governance-and-development/fips.mdx) (FIPs). 

The native token of the network is called MIX. New MIX tokens are issued with every added block and used to pay for gas, like Ether (ETH) on Ethereum. For more information about issuing new tokens, please refer to the [tokenomics](fuse-token.mdx#fuse-tokenomics) section.
