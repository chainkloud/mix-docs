...
The [Multichain](https://multichain.org) Multichain bridge platform can transfer MIX between Fuse and Polygon. It is also available for transferring three other tokens: agEUR (Angle), BIFI (Beefy Finance), and OLE (OpenLeverage).&#x20;

Although it nominally also supports ETH and USDC transfers, we advise against doing that as the canonical Fuse versions of those assets (with significant liquidity on the Mix Network) are currently not supported.
