...
Successfully running a node? Follow these steps to update your metadata to be displayed on [Staking App](https://stakingv2.miexs.com/ "Staking App"):
...
* If you want to update your metadata, search for your node address, and make changes to the attribute next to it, currently 4 attributes are supported:
  ...
  2. **image**:- Location of your node's image, all images are located in `src/assets/logos` you are required to update/add the logo in the same directory, and your image attribute should look like `logos/<image name>` , The recommended image size is\*\* 64px\*64px or 128px\*128px in jpg/png\*\* format.
...
On the [original repository](https://github.com/fuseio/fuse-staking-v2) page, you should see an option to compare and open a new pull request. Click on it, and select your branch as the source branch and the target branch as `fuseio/fuse-staking-v2 master` where we want it merged, provide a short description, and ask for a review. If the review is a success, your PR will be merged and changes will be live within a few minutes of merging.
