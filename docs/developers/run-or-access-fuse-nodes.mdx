---
title: Run Or Access Mix Nodes
sidebar_position: 5
---

# Run or Access Mix Nodes
...
The quickstart script can make starting a node for the MixNetwork as quick as possible.

1. Download the script.
2. Download one of the example `.env` files located in the [examples folder](https://github.com/chainkloud/mix-network/tree/master/scripts/examples).
...
$ wget -O .env https://raw.githubusercontent.com/chainkloud/mix-network/master/scripts/examples/.env.<ROLE>.example
$ ./quickstart.sh
```
...
To run the parity client for the Mix Network, you first have to pull the image from [DockerHub](https://hub.docker.com/r/fusenet/node).
...
#   The key-set is expected to to be placed in the custom configuration folder under 'keys/MixNetwork/'
...
Inside a directory for the keys with another sub-directory for the MixNetwork chain is used by _Parity_.
...
[Check out this section](https://github.com/chainkloud/mix-network/blob/master/README.md#create-new-account) if you have no account or want to create a new one.
...
```
$ mkdir -p ./config/keys/MixNetwork
$ cp /path/to/my/key ./config/keys/MixNetwork/
...
One is the [validator-app](https://github.com/chainkloud/mix-network/tree/master/app)
...
Concerning the instructions for the [validator](https://github.com/chainkloud/mix-network/blob/master/README.md#validator) role, we use the folder called `config` to bind as _Docker_ volume to _Parity_.
...
All roles should also run an [Ethereum Network Intelligence API](https://github.com/fuseio/eth-net-intelligence-api) app as well, to connect themselves as part of the network and be viewed by the [health](https://status.miexs.com/) service
